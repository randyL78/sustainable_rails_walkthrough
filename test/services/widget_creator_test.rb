...
class WidgetCreatorTest < ActiveSupport::TestCase
  ...
  test "name, price, and manufacturer are required" do
    ...
    assert widget.errors[:manufacturer_id].any? { |message|
      message =~ /can't be blank/i
    }, widget.errors.full_messages_for(:manufacturer_id)
  end

  test "price cannot be zero" do
    result = @widget_creator.create_widget(
        Widget.new(
            name: "Stembolt",
            price_cents: "0",
  ...
  test "price can't be more than $10,000" do
    result = @widget_creator.create_widget(
        Widget.new(
            name: "Stembolt",
